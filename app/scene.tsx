...
import {SYSTEMS,withBase,type Atlas,type SceneState} from './anatomy';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 const translator=useRef(t);translator.current=t;
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(translator.current('scene.webglError'));return;}
  ...
  renderer.domElement.setAttribute('aria-label',translator.current('scene.aria'));
  ...
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci],compressed=!!chunk.gzip&&typeof DecompressionStream!=='undefined';const response=await fetch(compressed?withBase(chunk.gzip!):withBase(chunk.url),{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error(translator.current('scene.geometryError'));geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:translator.current('scene.loadError'));}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(translator.current('scene.contextLost'));};renderer.domElement.addEventListener('webglcontextlost',contextLost);
